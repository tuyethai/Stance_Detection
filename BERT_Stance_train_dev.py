...
# df = pd.read_csv("../StanceDataset/test.csv", delimiter=',', lineterminator='\n', encoding="utf-8",
#                   header=0, names=['Tweet', 'Target', 'Stance', 'Opinion Towards', 'Sentiment'])
...
# df.loc[df.Sentiment == 'neg'].sample(5)[['Tweet', 'Target']]
...
max_seq_len = 512
# max_seq_len_tweets = 480#max([len(sen) for sen in Tweets])
# max_seq_len_targets= 25# max([len(sen) for sen in Targets])
                         
# print('Max sentence length in Tweets: ', max_seq_len_tweets)
# print('Max sentence length in Targets: ', max_seq_len_targets)
print(labels.shape)
...
# word2index = ""
# dict_class_labels = {'AGAINST':0, 'FAVOR':1, 'NONE':2}
# labels = [dict_class_labels[label] for label in labels]
...
#         scheduler.step()
    ...
    print(pred_flat)
    print(labels_flat)
#     iprecision, irecall, ifscore, _ = precision_recall_fscore_support(labels_flat,pred_flat, average=None)
    ...
    if epoch_i%save_step==0:
        ...
        list_data.append({'epoch':epoch_i,'train_loss':total_loss, 'val_acc': val_acc})
